...
class Dataset(object):
    def __init__(self, split):
        ...
        self.all_videos = [os.path.join(args.data_root, vid) for vid in video_list]
    ...
    def __getitem__(self, idx):
        ...
        for attempt in range(max_attempts):
            try:
                ...
                print(f"[DEBUG] intentando cargar: {vidname}")
                print(f"[DEBUG] num imágenes: {len(img_names)}")
            
                try:
                    wav = self.get_wav(wavpath)
                except Exception as e:
                    print(f"[DEBUG] fallo al cargar audio: {e}")
                    return None

                print(f"[DEBUG] longitud audio: {len(wav)} muestras")
            
                min_length = min(len(img_names), math.floor(len(wav) / 640))
                lastframe = min_length - v_context
            
                print(f"[DEBUG] min_length = {min_length}, lastframe = {lastframe}")
            
                if lastframe <= 0:
                    print("[DEBUG] lastframe <= 0, descartando ejemplo")
                    return None
                ...
                if window_fnames is None:
                    print("[DEBUG] get_window() devolvió None")
                    return None

                print(f"[DEBUG] tamaño ventana: {len(window_fnames)} frames")
                ...
                for fname in window_fnames:
                    ...
                    if img is None:
                        print(f"[DEBUG] imagen no se pudo leer: {fname}")
                        all_read = False
                        break
                    try:
                        img = cv2.resize(img, (hparams.img_size, hparams.img_size))
                        print(f"[DEBUG] Tamaño de la imagen después de redimensionar: {img.shape}")
                    except Exception as e:
                        print(f"[DEBUG] fallo al redimensionar {fname}: {e}")
                        all_read = False
                        break

                    window.append(img)
                if not all_read:
                    print("[DEBUG] al menos una imagen falló, descartando ejemplo")
                    return None

                if len(window) == 0:
                    print("[DEBUG] ventana vacía, descartando ejemplo")
                    return None
                ...
                try:
                    ...
                    print(f"[DEBUG] fallo al concatenar: {e}")
                    return None
                print(f"[DEBUG] Total de elementos en vid: {vid.size}")
                ...
                try:
                    ...
                    print(f"[DEBUG] fallo al calcular mel espectrograma: {e}")
                    return None
            
                if torch.any(torch.isnan(vid)) or torch.any(torch.isnan(mels)):
                    print("[DEBUG] contiene NaNs, descartando")
                    return None
                if vid==None or mels==None:
                    continue

                print(f"[DEBUG] ejemplo válido, retornando con lastframe={lastframe}")
                return vid, mels, lastframe
            except Exception as e:
                print(f"[DEBUG] intento {attempt+1}/{max_attempts} fallido: {e}")
                continue
        raise IndexError(f"[ERROR] No se pudo cargar ejemplo válido tras {max_attempts} intentos.")
...
def eval_model(test_data_loader, device, model):
    ...
    print(f"[DEBUG] total batches en DataLoader: {len(test_data_loader)}") 
    
    for step, (vid, aud, lastframe) in prog_bar:
        print(f"[DEBUG] STEP {step} OK")
        model.eval()
        with torch.no_grad():
